...
(define option-dup
  (lambda (option aux)
    (if (eq? option null)
        ...
        (option-dup (cdr option) (append aux (list (id-option option))))
        (option-dup (cdr option) aux)))))
    (cond
      [(null? option) (list id name (list ))]
      [(= (length option) 1) (list id name option)]
      [else (list id name (option-dup option (list )))])))
#|
Nombre de la función: option-dup
Dominio: option x aux. 
Recorrido: aux.
Descripción: Esta función toma como valores de entrada las options que provienen de la función flow y un aux que nos va a servir para guardar
